...
export async function userBoard(token) {
    ...
    /* --------------------------- 3 merge, format, default sort -------------------------------- */
    const data = users
        .map(u => {
            const ev = u.events_aggregate?.nodes?.[0];
            if (!ev || !ev.userAuditRatio) return null;      // skip empty rows
        ...
        .filter(Boolean)
        .sort((a, b) => b.level - a.level);  // Level desc by default

    /* ------- 4 helper to build one row (with sortable data‑attrs) -------------------------------- */
    const rowHTML = (u, idx) => `
